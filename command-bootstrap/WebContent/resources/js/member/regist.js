...
function picture_go() {
	...
	var fileFormat = picture.value.substr(picture.value.lastIndexOf(".") + 1)
			.toUpperCase();
...
function upload_go() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			// 지정된 파일명 저장
			$('input#oldFile').val(data); // 변경시 삭제될 파일명 // handler에서
											// out.print 한 데이터가 들어온다.
